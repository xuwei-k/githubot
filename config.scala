...
new Config {
  ...
  override val filter = { (u: UserAction) =>
    val s = u.tweetString
    List("codecov-io", "googlebot", "asfgit", "grpc-jenkins", "netkins", "buildhive", "scala-jenkins", "akka-ci", "asfbot", "ConfluentJenkins")
      .forall(disable => !s.contains(disable))
  }
  ...
  private def splitLine(str: String): String = {
    val delimiters = List(' ', '。', '、', '　', ' ')
    val words: Iterator[String] = delimiters
      .foldLeft(Array(str.trim)) { (strings, c) =>
        strings.flatMap(_.split(c))
      }
      .iterator
  ...
  def action2html(action: UserAction): String = {
    ...
    action.content.lines.zipWithIndex.filterNot { case (line, index) =>
